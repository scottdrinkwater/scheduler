...
const nextOccurrence = (period: Period, date: Date) => {
    switch (period.unit) {
      case "day": return addDays(date, period.measure);
      case "week": return addWeeks(date, period.measure);
      case "month": return addMonths(date, period.measure);
      case "year": return addYears(date, period.measure);
    }
}

const occurrences = <TEvent>(pattern: Pattern<TEvent>, startDate: Date, endDate: Date): Occurrence<TEvent>[] => {
    const concreteEnd = min([pattern.endDate ?? endDate, endDate]);
    const occurrences: Occurrence<TEvent>[] = [];
    for (let date = pattern.startDate; date < concreteEnd; date = nextOccurrence(pattern.frequency, date)) {
...
export const scheduler = <TEvent>(
  ...
  return {
    ...
    calendar: (startDate: Date, endDate: Date): Occurrence<TEvent>[] => {
      return schedule.patterns.flatMap((pattern) => occurrences(pattern, startDate, endDate))
    }
  };
};
